...
export default {
  ...
  data() {
    return {
      ...
      ScoreEditTemplate: function() {
        return {
          template: Vue.component("ScoreEditTemplate", {
            template: `
            <span>
             {{data.Score}}
            </span>
      ...
      LevelID: '',
      dayweekly: '',
      toolbarOptions : ['Add','Cancel'],
      editSettings:  { allowAdding: true, allowDeleting: false, allowEditing: true, mode: 'Normal',allowEditOnDblClick: true },
  ...
  methods: {
    ...
    AddScorePerfomance(data){
       const mObj = {
          KPILevel_ID : data.ID,
          KPILevel_Code : data.KPILevelCode,
        }
        axios.post('AdminKPILevel/AddScorePerfomance',JSON.stringify(mObj))
        .then(res=>{
          this.dataConversion = res.data.data
        })
      // axios.get(`AdminKPILevel/AddScorePerfomance/${data.ID}`).then(res=>{
      //   this.dataConversion = res.data.data
      // })
    },
    actionBegin(args) {
      if (args.requestType === "save") {
        if (args.action === "edit") {
          ...
          axios.post('AdminKPILevel/UpdateScorePerfomanceData',JSON.stringify(mObj))
          .then(res=>{
