              ...
              <div>
                <ejs-grid  ref="grid" :gridLines='lines' :allowTextWrap='true' :allowResizing='true' :editSettings='editSettings' :actionBegin='actionBegin' :searchSettings="searchSettings" :rowSelected="rowSelected"  :toolbar='toolbarOptions' :dataSource='dataActionPlan' :allowPaging="true" :pageSettings='pageSettings' >
                  <e-columns>
                    ...
                    <e-column field='ID' headerText='Task ID' isPrimaryKey='true' width='120' textAlign='Center' ></e-column>
                    <e-column field='Title' :template="TitleTemplate" headerText='Task name' :edit="editparams"  width='200' textAlign='Center' ></e-column>
...
export default {
  ...
  watch: {
    ...
    categoryname: function(newOld,oldVal){
      ...
      // var item2 = $(".RemarkChart2").text().replace("-  -",' - '+newOld + ' - ');
      //   $(".RemarkChart2").text(item2)
  ...
  methods: {
    ...
    btntabload(item) {
      ...
      console.log(seft.commentID)
      console.log(seft.dataID)
    ...
    loadDataComment(id) {
      let seft = this;
      $.ajax({
        url: `http://10.4.5.132:991/ChartPeriod/LoadDataComment/${id}/${VueJwtDecode.decode(localStorage.getItem("authToken")).nameid}`,
        type: "GET",
        dataType: "json",
