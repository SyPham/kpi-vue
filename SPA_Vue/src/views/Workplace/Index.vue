              ...
              <div class="card-header with-border kpi-name">
                ...
                <div class="box-tools float-right toolActionplan">
                  ...
                  <div class="btn-group">
                    <button  type="button" value="false" class="btn btn-danger btn-sm button-nocolor NotFinished">
                      <i class="fa fa-close"></i> Not Approved
                    </button>
                  </div>
                </div>
                <div class="form-group">
                  <select id="role" class="form-control role" style="width:200px">
                    ...
                    <option value="Man">{{$t("Manager_label")}}</option>
                    <option value="Own">{{$t("Owner_label")}}</option>
                    <option value="Aud">Auditor</option>
                    <option value="Pic">PIC</option>
                    <option value="Upd">{{$t("Updater_label")}}</option>
                    <option value="Spo">{{$t("Sponsor_label")}}</option>
                    <option value="Par">{{$t("Participant_label")}}</option>
                  </select>
                </div>
              </div>
              <div class="card-body">
...
export default {
  data() {
    return {
      ...
      pageSize3: 6,
      changePageSize: " ",
      url:"http://10.4.5.174:91/Workplace/ExcelExport/" + VueJwtDecode.decode(localStorage.getItem("authToken")).nameid
      // url:"http://10.10.0.34:91/Workplace/ExcelExport/" + VueJwtDecode.decode(localStorage.getItem("authToken")).nameid
    };
  },
  components: {
    Paginate
      ...
      $(document).ready(function() {
        ...
        setTimeout(function(){
          ...
          $("#treetable").fancytree({
            ...
            glyph: glyph_opts,
            source: {
              url:"http://10.4.5.174:91/KPI/GetListTreeClient/" + VueJwtDecode.decode(localStorage.getItem("authToken")).nameid,
              // url:"http://10.10.0.34:91/KPI/GetListTreeClient/" + VueJwtDecode.decode(localStorage.getItem("authToken")).nameid,
              debugDelay: 1000
            },
            table: {
              indentation: 20,
