...
export default {
  ...
  methods: {
    ...
    getAllKPILevelByCategory(changePageSize) {
      ...
      axios.get(
        `CategoryKPILevel/getAllKPILevelByCategory2/${self.categoryId}/${self.level}/${self.locale}/${self.pageIndex}/${self.pageSize}`
      ).then(response => {
    ...
    switchChartPriod(period, kpilevelcode) {
      ...
      switch (period) {
        case "W":
          ...
          // self.$router.push( `/ChartPeriod/${kpilevelcode}/${this.categoryId}/${period}/${year}/1/53` );
          break;
        case "M":
